      ...
      <header className="sticky top-0 z-30 w-full bg-transparent px-4 py-3 sm:px-6 lg:px-8 transition-all duration-300">
        <div
          className={`mx-auto max-w-7xl h-16 rounded-2xl flex items-center justify-between px-6 transition-all duration-350 ${
            isHome && !scrolled
              ? "bg-navy/35 backdrop-blur-md border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.2)]"
              : "bg-white/80 backdrop-blur-md border border-border/80 shadow-elevated"
          }`}
        >
          {/* Mobile menu button */}
          <button
          ...
          <Link href="/" className="flex items-center">
            <img
              src={isHome && !scrolled ? "/logo_mesolab_pro_h_web-white.svg" : "/logo_mesolab_pro_h_web.svg"}
              alt="MesoLab Pro"
          ...
          <nav className="hidden items-center gap-8 lg:flex" aria-label="Navegación principal">
              ...
              >
                ...
                <svg
                  className={`h-4.5 w-4.5 transition-transform duration-200 ${storeHovered ? "rotate-180" : ""} ${
                    isHome && !scrolled ? "text-white/60" : "text-muted/60"
                  }`}
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
            ...
            <Link
              ...
              className={`text-sm font-medium transition-colors ${
                isHome && !scrolled ? "text-white/80 hover:text-white" : "text-navy/80 hover:text-teal-dark"
              }`}
            >
              Nosotros
            </Link>
          ...
          <div className="flex items-center gap-3">
            ...
            <a
              ...
              className={`hidden items-center gap-2 rounded-[var(--radius-md)] px-4 py-2 text-sm font-medium transition-colors lg:flex ${
                isHome && !scrolled
                  ? "bg-white/10 text-white hover:bg-white/20 border border-white/10"
                  : "bg-navy text-white hover:bg-navy-light"
              }`}
            >
              <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" className={isHome && !scrolled ? "text-whatsapp" : ""}>
        ...
        <AnimatePresence>
          {mobileMenuOpen && (
            <motion.div
              ...
              className={`overflow-hidden lg:hidden rounded-2xl mt-2 border p-3 shadow-modal backdrop-blur-md transition-all duration-300 mx-auto max-w-7xl ${
                isHome && !scrolled
