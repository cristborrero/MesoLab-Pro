...
export function Header() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // En la home: transición casi al final del hero (~80% del viewport height)
      // En otras páginas: transición rápida a los 60px
      const threshold = isHome ? window.innerHeight * 0.78 : 60;
      setScrolled(window.scrollY > threshold);
    };
    // Ejecutar de inmediato para estado inicial correcto en subpáginas
    handleScroll();
  ...
  // Estado activo: transparente solo en Home sin haber scrolled suficiente
  const isTransparent = isHome && !scrolled;

  return (
    <header className="fixed top-0 left-0 right-0 z-30 w-full px-4 pt-4 pb-2 sm:px-6 lg:px-8">
      {/* Píldora de navegación principal */}
      <motion.div
        className={`w-full mx-auto max-w-screen-xl h-16 rounded-2xl flex items-center justify-between px-5 lg:px-8 border ${
          isTransparent
            ? "bg-navy/30 border-white/15 shadow-[0_8px_40px_rgba(0,0,0,0.25)]"
            : "bg-white/85 border-white/60 shadow-[0_4px_24px_rgba(0,0,0,0.08)]"
        }`}
        style={{ backdropFilter: "blur(16px)", WebkitBackdropFilter: "blur(16px)" }}
        animate={{
          backgroundColor: isTransparent ? "rgba(10,25,47,0.30)" : "rgba(255,255,255,0.85)",
        }}
      ...
      >
        ...
        <button
          ...
          className={`flex h-10 w-10 items-center justify-center rounded-md transition-colors lg:hidden ${
            isTransparent ? "text-white hover:bg-white/10" : "text-navy hover:bg-surface"
          }`}
        ...
        {/* Desktop Nav Links con Mega Menú */}
        <nav className="hidden items-center gap-8 lg:flex" aria-label="Navegación principal">
            ...
            <Link
              ...
              className={`flex items-center gap-1 text-sm font-medium transition-colors duration-300 ${
                isTransparent
                  ...
                  : "text-navy/80 hover:text-teal-dark"
              }`}
            >
              Tienda
              <svg
                className={`h-4.5 w-4.5 transition-transform duration-200 ${storeHovered ? "rotate-180" : ""} ${
                  isTransparent ? "text-white/60" : "text-muted/60"
                }`}
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
          ...
          <Link
            ...
            className={`text-sm font-medium transition-colors duration-300 ${
              isTransparent ? "text-white/80 hover:text-white" : "text-navy/80 hover:text-teal-dark"
            }`}
          >
            Nosotros
          </Link>
        ...
        <div className="flex items-center gap-3">
          ...
          <a
            ...
            className={`hidden items-center gap-2 rounded-[var(--radius-md)] px-4 py-2 text-sm font-medium transition-all duration-300 lg:flex ${
              isTransparent
                ? "bg-white/10 text-white hover:bg-white/20 border border-white/15"
                : "bg-navy text-white hover:bg-navy-light"
            }`}
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" className={isTransparent ? "text-[#25D366]" : ""}>
          ...
          <button
            onClick={openCart}
            className={`relative flex h-10 w-10 items-center justify-center rounded-md transition-colors duration-300 ${
              isTransparent ? "text-white hover:bg-white/10" : "text-navy hover:bg-surface"
            }`}
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className={`overflow-hidden lg:hidden rounded-2xl mt-2 border p-3 shadow-modal backdrop-blur-md w-full mx-auto max-w-screen-xl ${
              isTransparent
                ? "bg-navy/95 border-white/10 text-white"
                : "bg-white/95 border-border/80 text-navy"
            }`}
          >
            <nav className="flex flex-col gap-1 px-2 py-2" aria-label="Menú móvil">
              {NAV_LINKS.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  ...
                  className={`rounded-md px-3 py-2.5 text-sm font-medium transition-colors ${
                    isTransparent
                      ...
                      : "text-navy/80 hover:bg-surface hover:text-teal-dark"
                  }`}
                >
                  {link.label}
                </Link>
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className={`mt-2 flex items-center justify-center gap-2 rounded-[var(--radius-md)] px-3 py-2.5 text-sm font-medium transition-colors ${
                  isTransparent
                    ? "bg-white/10 text-white hover:bg-white/20 border border-white/15"
                    : "bg-navy text-white hover:bg-navy-light"
                }`}
